...
We ran 25 seasons across all five draft mechanisms with all 30 teams
represented by \texttt{claude-haiku-4-5}, using the same payoff parameters
as the rational-agent baseline ($V = 200$).
Table~\ref{tab:llm_results} summarizes the results.

\begin{table}[h]
\centering
\begin{tabular}{lrrr}
\hline
\textbf{Mechanism} & \textbf{Overall tank\%} & \textbf{When out\%} & \textbf{Late-season\%} \\
\hline
NBA Lottery (2019)     & 8.0\% & 17.1\% & 11.5\% \\
Bilevel                & 6.2\% & 13.3\% &  4.6\% \\
COLA                   & 0.0\% &  0.0\% &  0.0\% \\
Weighted Loss (exp)    & 3.4\% &  7.2\% &  2.4\% \\
NBA 3-2-1              & 0.0\% &  0.1\% &  0.0\% \\
\hline
\end{tabular}
\caption{LLM agent tanking rates by mechanism (25 seasons, $V=200$).
``When out'' conditions on decisions where the team's rank exceeds the playoff cutoff.
``Late-season'' conditions on decisions after game~60 of 82.}
\label{tab:llm_results}
\end{table}

Two distinct behavioral patterns emerge.
The first is \emph{Type~1 tanking}: strategic effort reduction driven by
lottery incentives. This is what the rational agent exhibits.
The second is \emph{Type~2 tanking}: behavioral post-elimination effort
reduction, in which the LLM reduces effort once playoff contention is
perceived as remote, independent of the specific lottery payoff structure.
Type~2 tanking explains why NBA Lottery LLM rates (8.0\%) remain somewhat
above the rational-agent baseline (6.9\%): the LLM applies a sharper
qualitative elimination threshold than continuous expected-utility maximization.

The mechanism-specific results reveal that LLM agents correctly respond
to the incentive structures embedded in each mechanism's design.

\textbf{COLA and NBA 3-2-1} produce near-zero tanking (0.0\% and 0.1\%
respectively). Under COLA, every non-playoff team earns the same single
ticket regardless of record, so the LLM correctly identifies no within-season
lottery benefit from tanking. Under 3-2-1, the LLM correctly identifies that
non-playoff teams have no lottery reason to tank: Tier~A (bottom~3) teams
receive \emph{fewer} balls than Tier~B, and within each tier all teams hold
equal ball counts regardless of their relative rank.

\textbf{Bilevel} exhibits the lock effect clearly.
Mid-season tanking (games 30--60) runs at 12.0\%, consistent with
NBA-lottery-like incentives before the game-70 standings lock.
After game~60, tanking drops to 4.6\% as the LLM receives the signal that
draft position has been frozen and further effort reduction is worthless.
The residual 4.6\% reflects Type~2 behavioral elimination responses that
persist even after the lottery incentive has been removed.

\textbf{Weighted loss} displays the predicted decay signature.
Mid-season tanking (6.1\%) is well below NBA Lottery mid-season rates (11.9\%),
reflecting the LLM's awareness that losses become progressively less valuable.
Late-season tanking (2.4\%) is substantially below NBA Lottery late-season
rates (11.5\%), confirming that the exponential decay successfully communicates
declining incentives to qualitative reasoners.

Representative reasoning traces illustrate Type~2 behavioral tanking:

\begin{quote}
\textit{``At 24-38 with 20 games left, playoff probability is negligible
(${\sim}$2\%); projecting to ${\sim}$28-29 wins places us firmly outside
contention; minimize effort to improve lottery position.''}
(Weighted Loss, season~12, $e = 0.0$)
\end{quote}
...
(NBA Lottery, season~3, $e = 1.0$)
\end{quote}

One important caveat applies to all LLM results.
The near-zero tanking rates under COLA and 3-2-1 are conditional on the
fixed playoff value assumption ($V = 200$).
In a season featuring a generational draft talent, the expected value of
any lottery ticket rises dramatically, and the playoff-versus-lottery
tradeoff can flip even under mechanisms designed to eliminate tanking
incentives.
Our results therefore characterize LLM behavior under typical draft-class
conditions; they should not be interpreted as mechanism-proof resistance
to tanking in all circumstances.
...
First, extending the model to incorporate variable draft class strength
would allow testing whether reform mechanisms remain robust when the expected
value of high picks rises sharply, as in generational draft years.
Our LLM results show near-zero tanking under COLA and 3-2-1 given the
fixed playoff value assumption ($V = 200$); a sufficiently strong draft class
could flip this result even under anti-tanking mechanisms.
Second, the structural question raised by our results about competitive
...
Third, a hybrid of the bilevel and 3-2-1 mechanisms warrants formal study.
