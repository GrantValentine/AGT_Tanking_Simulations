...
plus = [
    ...
    ("★", "LLM agents (25 seasons)",
     "All 30 = Claude Haiku\n5 mechanisms",
     "Do LLMs discover mechanism-specific tanking strategies?"),
]
...
bullet_box(slide, [
    ...
    "Limitation: expensive (25 seasons of 30 teams × ~7 decisions)",
],
...
results_slide(
    title="Result 5: LLM Agents  (25 seasons, all 5 mechanisms)",
    subtitle="LLM tanking is mechanism-dependent — range 0% to 8% across designs",
    fig_path=FIGS / "fig8_rational_vs_llm_by_mechanism.png",
    takeaways=[
        ("LLM tanking rate by mechanism:", True),
        "---",
        "  COLA:           0.0%  ← full IC; LLM understands",
        "  NBA 3-2-1:     ~0.0%  ← inverted incentive recognized",
        "  Weighted loss:  3.4%  ← decay understood; some early-season",
        "  Bilevel:        6.2%  ← mid-season lock effect visible",
        "  NBA lottery:    8.0%  ← highest; behavioral elimination heuristic",
        "---",
        ("Two types of LLM tanking:", True),
        "Type 1: lottery-driven (requires mechanism incentive)",
        "Type 2: post-elimination effort drop (mechanism-independent)",
        "LLMs mostly show Type 2: 'playoff odds negligible → reduce effort'",
        "---",
        ("Draft class caveat:", True),
        "Results assume playoff value > lottery EV  (V = 200)",
        "Generational draft year: lottery EV spikes → could flip even",
        "COLA and 3-2-1 from near-zero to meaningful tanking",
    ],
    chart_w_frac=0.52,
)
...
bullet_box(slide, [
    ...
    "---",
    ("4. LLM tanking is mechanism-dependent  (0% → 8% range)", True),
    "   COLA and 3-2-1: near-zero; Bilevel and NBA lottery: higher",
    "   Type 2 behavioral tanking (post-elimination effort drop) dominates",
    "---",
...
bullet_box(slide, [
    ("Draft class strength  (links to LLM results)", True),
    "We hold D(k) fixed; playoff value dominates throughout",
    "Generational draft year: D(1) spike could flip COLA / 3-2-1",
    "Anti-tanking guarantees may not hold in unusually strong draft years",
    "---",
