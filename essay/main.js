...
function initLottery() {
  ...
  const sim = d3.forceSimulation(nodes)
    ...
    .force("y", d3.forceY(H * 0.52).strength(0.22))   // stronger gravity pull
  ...
  window._updateLotteryStep = function(step) {
    ...
    if (step === 0) {
      ...
      circles.attr("pointer-events", "all")
        .on("mouseover", (event, d) => { showClusterInfo(d.rank); })
        .on("mouseout", () => infoBox.attr("opacity", 0));
      ...
      sim.stop();
      infoBox.attr("opacity", 0);
      circles.attr("pointer-events", "none").on("mouseover", null).on("mouseout", null);
      document.getElementById("lottery-legend").innerHTML = "";
...
function initPickValues() {
  ...
  const W = Math.min(el.clientWidth || 460, 460), H = 260;
  ...
  // Dynamic player-specific rating line
  const playerLineG = g.append("g").attr("opacity", 0);
  const playerLineEl = playerLineG.append("line")
    .attr("x1", 0).attr("x2", iw)
    .attr("stroke", "rgba(255,255,255,0.7)").attr("stroke-width", 1.5)
    .attr("stroke-dasharray", "5,3");
  const playerLineLbl = playerLineG.append("text")
    .attr("x", iw + 4).attr("text-anchor", "start")
    .attr("fill", "rgba(255,255,255,0.75)").attr("font-size", 8.5).attr("font-weight", 600);

  let activePlayerKey = null;

  function showPlayerLine(playerKey) {
    activePlayerKey = playerKey;
    const kp = nba.key_players.find(p => `${p.year}-${p.pick}` === playerKey);
    if (!kp) { playerLineG.attr("opacity", 0); return; }
    const ratingMap = { avg_peak: kp.peak_rating, avg_first: kp.first_rating, avg_mean: null };
    const rating = ratingMap[currentMetric] ?? kp.peak_rating;
    if (!rating) { playerLineG.attr("opacity", 0); return; }
    const yPos = yL(rating);
    playerLineEl.attr("y1", yPos).attr("y2", yPos);
    playerLineLbl.attr("y", yPos + 4).text(rating);
    playerLineG.transition().duration(400).attr("opacity", 1);
  }

  function hidePlayerLine() {
    activePlayerKey = null;
    playerLineG.transition().duration(300).attr("opacity", 0);
  }
  ...
  document.querySelectorAll(".chart-toggle-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (activePlayerKey) showPlayerLine(activePlayerKey);
    });
  });
...
function initTrajectory() {
  ...
  if (traj.decisions && traj.decisions.length > 0) {
    dotGroup.selectAll(".decision-dot")
      ...
      .attr("cx", d => x(d.total_games))  // FIX: use total_games
      .attr("cy", d => y(d.rank))
      .attr("r", 6)
      ...
      .on("mouseover", function(event, d) {
        ...
        tooltip.innerHTML = `
          ...
          ${decLabel}
          <div style="margin-top:0.3rem;opacity:0.85;font-size:0.8rem;font-style:italic;">${d.reasoning || "No reasoning logged."}</div>`;
...
function drawReformBilevel() {
  ...
  // Lock line — extend above bar so label sits clearly above it
  g.append("line")
  ...
  // "No lottery / benefit" — two lines, left-aligned just right of lock line
  g.append("text")
  ...
  // Game 70 label floats above the lock line
...
function initFairness() {
  ...
  // Tau dot plot (static)
  {
    ...
    tau.mechanisms.forEach((m2, i) => {
      ...
      g.append("text")
        .attr("x", x(m2) + x.bandwidth() / 2).attr("y", y(tau.rational_tau[i]) - 15)
        .attr("text-anchor", "middle").attr("fill", GRAY).attr("font-size", 11).attr("font-weight", 600)
